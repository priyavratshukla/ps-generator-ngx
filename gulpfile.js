...
const config = {
  libraryName: 'generator-ngx-library',
  unscopedLibraryName: 'generator-ngx-library',
  allSrc: 'src/**/*',
  allTs: 'src/**/!(*.spec).ts',
...
gulp.task('build:demo', () => {
  return execDemoCmd(`build --preserve-symlinks --prod --base-href /generator-ngx-library/ --deploy-url /generator-ngx-library/`, { cwd: `${config.demoDir}`});
});
...
gulp.task('build:demo-ssr', () => {
  return execDemoCmd(`build --preserve-symlinks --prod`, { cwd: `${config.demoDir}`})
    .then(() => execDemoCmd(`run generator-ngx-library-demo:server`, { cwd: `${config.demoDir}` }))
